...
module.exports = function betterThanFT(name, value) {
	const count = `SELECT COUNT(current_values.value) as amount FROM current_values JOIN page ON page.id = current_values.page_id JOIN properties ON properties.id = current_values.property_id WHERE current_values.page_id NOT IN (SELECT id FROM page WHERE domain='www.theguardian.com' OR domain='international.nytimes.com' OR domain='www.nytimes.com' OR domain='www.wsj.com') AND properties.name = ${escape(name)};`;
