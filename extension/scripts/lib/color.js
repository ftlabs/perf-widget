...
(function(){
	...
	const _ = self.Color = function(rgba) {
		if (rgba === 'transparent') {
			rgba = [0,0,0,0];
		}
	...
	_.prototype = {
		...
		contrast: function (color) {
			...
			const onBlack = this.overlayOn(_.BLACK).contrast(color).ratio;
			const onWhite = this.overlayOn(_.WHITE).contrast(color).ratio;
			...
			const min = this.overlayOn(closest).contrast(color).ratio;
